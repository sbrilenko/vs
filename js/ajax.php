...
switch($_POST['action'])
{
    ...
    if(!empty($phone) AND !empty($fio) AND !empty($address))
    {
        ...
        /*foreach($_POST['a'] as $index=>$val)
         ...
         {
            $id_cl=$db->getArray();
            $sql="UPDATE clients SET name='{$fio}',address='".$address."' WHERE id='".$id_cl[0]['id']."' AND updated=1";
                ...
                foreach($_SESSION['arrayorder'][$i] as $index=>$val)
                {
                    if(!empty($val))
                    {
                        ...
                        if($db->getCount()>0)
                        {
                            ...
                            $sql_goods="SELECT factor FROM unitsofgoods WHERE id='".$user_iz."' AND idgoods='".$val['id']."'";
                            
                            $db->query($sql_goods);
                            if($db->getCount()>0)
                            {
                                $arr=$db->getArray();
                                $factor_user=$arr[0]['factor'];
                            }
                            else
                            {
                                $factor_user=1;
                        ...
                        if($db->getCount()>0)
                        {
                            ...
                            $sql_goods="SELECT factor FROM unitsofgoods WHERE id='".$ed_iz."' AND idgoods='".$val['id']."'";
                            $db->query($sql_goods);
                            if($db->getCount()>0)
                            {
                                $arr=$db->getArray();
                        ...
                        $db->query($sql_put_to_ids);
                        $sql="INSERT INTO compositionoforder (id,idorder,idgoods,count,factor,price,sum,id_ed_iz,`updated`) VALUES ('w".$new_idorder_more."','w".$new_or_id."','".$val['id']."',".($val['k']*$factor_user).",".$factor.",".($price*$factor_user).",".($price*$factor_user*$val['k']).",'".$ed_iz."',1)";
                        $db->query($sql);   
                    }
                }
            }
